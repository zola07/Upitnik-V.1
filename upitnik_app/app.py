...
def preuzmi_pdf_fajl():
   # Provera da li korisnik ima jedinstveni ID u sesiji
    ...
    class PDF(FPDF):
        ...
        def footer(self):
            # Postavite font i veličinu teksta za podnožje
            self.add_font("DejaVuSans", fname="upitnik_app/static/font/DejaVuSans.ttf", uni=True)
            self.set_font("DejaVuSans", size=8)
            # Dodajte broj trenutne stranice i ukupan broj stranica
   ...
   # Kreirajte PDF koristeći FPDF
    ...
    pdf.add_font("DejaVuSans-Bold", fname="upitnik_app/static/font/DejaVuSans-Bold.ttf", uni=True)
    pdf.set_font("DejaVuSans-Bold", size=14)

    # Naslov
    pdf.cell(0, 15, txt="УПИТНИК", ln=0, align="L")

    # Logo (desno)
    pdf.image('upitnik_app/static/pictures/logo-nitra-hor-cir.png', x=140, y=5, w=60)

    # Dodatni tekst
    pdf.set_y(40)
    pdf.cell(0, 10, txt="Етичке смернице за развој, примену и употребу поуздане и", ln=1, align="C")
    pdf.cell(0, 10, txt="одговорне вештачке интелигенције", ln=1, align="C")
    pdf.set_font("DejaVuSans-Bold", size=12)
    pdf.cell(0, 10, txt="", ln=2, align="L")
    pdf.cell(0, 10, txt="ПДФ извештај", ln=2, align="C")
    pdf.add_font("DejaVuSans", fname="upitnik_app/static/font/DejaVuSans.ttf", uni=True)
    pdf.set_font("DejaVuSans", size=12)

    # Pretpostavka: render_template vraća vreme u formatu "yyyy-mm-dd"
    vreme_iz_templatea = vreme
    jedinstveni_id = jedinstveni_id

    # Pretvorite vreme iz "yyyy-mm-dd" u "d-m-y" format
    date_obj = datetime.strptime(vreme_iz_templatea, "%Y-%m-%d %H:%M:%S")
    formatirano_vreme = date_obj.strftime("%d-%m-%Y %H:%M:%S")

  
    # Postavite brojač stranica na početku
    pdf.set_y(85)
    pdf.set_font("DejaVuSans", size=12)
    pdf.cell(0, 10, f"Јединствени ИД корисника: {jedinstveni_id}", ln=3, align="L")
    pdf.cell(0, 10, f"Време попуњавања: {formatirano_vreme}", ln=4, align="L")
    pdf.set_font("DejaVuSans-Bold", size=12)
    pdf.set_y(110)
    pdf.cell(0, 10, txt="Ваши одговори", ln=6, align="C")
    pdf.set_y(120)  # Pomerite se niže kako biste počeli unositi tekst
    pdf.set_font("DejaVuSans", size=12)
